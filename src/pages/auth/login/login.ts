...
export class LoginPage {
  ...
  login() {
    ...
    if (!this.loginForm.valid) {
      ...
      if (!email.valid) {
        errorMessage = "Ops! correo invalido";
      } else if (!password.valid) {
        errorMessage = "La contraseña debe tener de 6 a 20 caracteres";
      }
      ...
      this.authData.loginUser(email, password).then(() => {
        ...
        this.loadingCtrl.dismiss().then(() => {
          this.alertCtrl.createWithError("Cuenta no encontrada, por favor verifique su correo  o contraseña");
        });
      });
    }
